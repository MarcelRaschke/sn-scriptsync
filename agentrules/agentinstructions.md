<!-- SN-SCRIPTSYNC:BEGIN instructionsSchemaVersion=24 -->
<!-- apiVersion: 24 -->
...
- **Plain data rows → `create_record`.** `create_artifact` requires `fields.name` and tracks the record in the local workspace, which only makes sense for scriptable artifacts. For an incident, task, `sys_user`, `cmdb_ci` or any table whose display field isn't `name`, use `create_record`: same `createArtifacts` gate, no name requirement, and it returns the inserted row so the write needs no separate read-back. Never create a record by driving the form UI (`navigate` + `set_field` + `run_ui_action`) — that path is for exercising real form behaviour, not for writing data.
...
- **`E_DISABLED` is a settings gate, not a bug.** Preflight all of them in one shot with `get_capabilities` → `gates` instead of discovering them mid-operation. The full set:
  - `createArtifacts` (`sn-scriptsync.createArtifacts.enabled`, **default on**) → `create_record`, `create_artifact`, `create_application`, `create_table`, `add_column`, `upload_attachment` (an upload inserts a `sys_attachment` row).
  - `updateRecords` (`sn-scriptsync.updateRecords.enabled`, **default on**) → `update_record`, `update_record_batch`, `run_ui_action` (committing the open form writes the record). Unset, it follows `createArtifacts`, so an instance that holds creates for approval holds updates too.
...
| `pull_scope` | Pull every scriptable artifact of one application scope to local files in one call (paged, refreshable). |
...
| `create_record` | Create a plain data row (incident, task, user) — never drive the form UI to do this. |
| `create_artifact` | Create a scriptable artifact incl. config fields via payload (not loose files). |
