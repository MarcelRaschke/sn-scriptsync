...
- **The agent connect snippet now points at the CLI instead of raw HTTP.** Copying the connect instructions used to hand an agent port-file discovery, a token header and a curl example — everything needed to hand-roll requests, and nothing that would validate them. It now names `@snutils/snu`, its one-line setup, and `snu context --json` as the first call, so an agent gets argument checking, review gating and structured errors by default. The raw HTTP contract is unchanged and still fully documented at `/api/instructions`.
- **The transport version and the instructions schema version no longer share a name.** One word, `apiVersion`, covered two unrelated numbers: the version of the HTTP/WebSocket contract reported by the bridge, and the schema version stamped into generated agent instructions. They are now `transportApiVersion` and `instructionsSchemaVersion`. **Nothing breaks:** `apiVersion` remains in the health response as an alias for older CLIs, and the extension keeps recognising instruction blocks written with the old marker permanently — a workspace whose `CLAUDE.md` carries `SN-SCRIPTSYNC:BEGIN apiVersion=N` is refreshed in place, never given a second block.
- **New `negotiate` command** answers what you are connected to in one response: transport version, host kind, extension version, connected SN Utils build and licence, and the authoritative list of commands that bridge supports. Checking that list beats discovering a missing command through an error mid-task.
- **An unsupported command now names what to upgrade.** `E_UNKNOWN_COMMAND` said the fix was to update either the extension or `@snutils/snu`, leaving the caller to work out which was serving. It now names the actual host and its version.
...
- **`snu context` no longer reports "Connected and ready" when ServiceNow has rejected the session.** Readiness used to be inferred entirely from local state — bridge up, listener up, helper tab attached — all of which stay true while every operation returns 401. A bounded read-only probe now answers the one question that matters, and the context report breaks out each link separately. A 403 is treated as authenticated: an ACL refusing a row is a different problem from an expired session.
- **New `snu auth status`** verifies the session for an instance, returning `AUTH_OK`, `AUTH_EXPIRED`, `AUTH_MISSING`, `HELPER_DISCONNECTED` or `INSTANCE_NOT_FOUND` instead of a generic 401. `snu auth refresh` explains the `/token` steps, since only you can refresh a session from the browser.
- **New `snu doctor`** collects versions, port descriptors, listeners, ownership, instance sessions and capabilities into one report, and names the actual problem — a stale descriptor, a healthy-but-unregistered bridge, a port held by something that is not serving, two workspaces claiming the bridge. It carries no tokens, session material or settings files.
- **MCP clients no longer stop each other's bridges.** Startup treated "discovery failed" as "no bridge exists", but discovery fails when the port descriptor is missing or stale — so a healthy bridge belonging to another MCP client was replaced. A running bridge is now proven by its health endpoint, and only a port that answers nothing may be reclaimed.
- **A wedged editor-hosted bridge can be taken over.** Holding the port was previously treated as owning it, so a window whose bridge had stopped answering blocked every other window with no way out but killing a process by hand. An owner that is demonstrably not serving now offers an explicit force-stop; a healthy one is still protected and still points you at the window that has it.
- **Fixed: a takeover left the bridge on a fallback port.** Port conflicts were resolved after the HTTP server had already bound, so a takeover lost port 1977 to the process being displaced and served on an ephemeral port until the next restart. Conflicts are now resolved before anything binds.
...
**See what an agent changed before you approve it (review mode):** Staged Agent API writes now show native diff gutter bars in the editor — open a pending file and the proposed-but-unapproved lines are flagged against the value still on the instance, with VS Code's inline "peek changes" a click away. Previously the staged write overwrote the file in place, so an open tab looked unchanged and there was nothing to diff against. Only appears while you have `sn-scriptsync.agentApi.reviewWrites` on and something staged.
