...
// `instructionsSchemaVersion=N`. Older builds emitted `apiVersion=N`; the
// extension's refresh regex accepts both, so existing workspaces keep matching.
...
const SKILL_MARKER = `<!-- SN-SCRIPTSYNC:SKILL instructionsSchemaVersion=${INSTRUCTIONS_VERSION} -->`;
...
function buildCore(): string {
	const header = [
		`<!-- SN-SCRIPTSYNC:BEGIN instructionsSchemaVersion=${INSTRUCTIONS_VERSION} -->`,
